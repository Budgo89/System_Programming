...
public class Fractal : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (li = 1; li < _parts.Length; li++)
        {
            ...
            MyJobForTransform myJobForTransform = new MyJobForTransform()
            {
                ...
                spinAngelDelta = spinAngelDelta,
                scale = scale,
            };
            JobHandle jobHandle1;
            var _transformAccess = new TransformAccessArray(AddTransform(levelParts));
            jobHandle1 = myJobForTransform.Schedule(_transformAccess);
            jobHandle1.Complete();
    ...
    private Transform[] AddTransform(NativeArray<FractalPart> levelParts)
    {
        var transforms = new Transform[levelParts.Length];
        for (int i = 0; i < levelParts.Length; i++)
        {
           transforms[i] = new RectTransform();
        }
    ...
    private struct MyJobForTransform : IJobParallelForTransform
    {
        ...
        public float spinAngelDelta;
        public float scale;

        public void Execute(int index, TransformAccess transform)
        {
            
            var parent = parentParts[index / _childCount];
            ...
            levelMatrices[index] = Matrix4x4.TRS(part.WorldPosition,
                part.WorldRotation, scale * Vector3.one);
        }
    }
